...
const redirect = {
	[AdminRole.MASTER_ADMIN]: "/master-franchise/dashboard",
	[AdminRole.SUPER_ADMIN]: "/super-franchise/dashboard",
	[AdminRole.REGIONAL_ADMIN]: "/regional-franchise/dashboard",
};

const PaymentProcessing = ({ updateRole }: { updateRole: AdminRole }) => {
	const { id } = useParams();
	...
	useEffect(() => {
		...
		const fetchStatus = async () => {
			try {
				...
				if (data.status === "SUCCESS" && data.parentAdminId) {
					...
					// Optional redirect
					router.push(redirect[updateRole as keyof typeof redirect]);
